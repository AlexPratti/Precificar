...
with tab2:
    ...
    with st.container(border=True):
        ...
        if categoria == "CABOS":
            ...
            nome_f, unidade = f"Cabo Flexível {sec} {cor}", "m"
            ...
            nome_f, unidade = f"Disjuntor {fase} {curva}{corr.replace(' A', '')}", "un"
            ...
            if categoria == "CONDUÍTES":
                ...
                tipo = st.selectbox("Tipo/Modelo:", ["C", "E", "X", "T", "LR", "LL", "LB", "TB", "B"])
                unidade = "un"
            qtd = c3.number_input("Qtde/Metros:", min_value=0.0, step=1.0)
            nome_f = f"{categoria.title()[:-1]} {sec} {tipo}"
            ...
            nome_f, unidade = desc, uni

        if st.button("➕ Adicionar à Lista"):
            if nome_f and qtd > 0:
                if adicionar_ou_somar_material(nome_f, qtd, unidade):
    ...
    if not mats_db:
        ...
        for item in mats_db:
            with st.container(border=True):
                ...
                n_edit = c1.text_input("Nome:", item['orc_item_nome_visual'], key=f"n_{item['id']}")
                q_edit = c2.number_input("Qtd:", value=float(item['orc_item_quantidade']), key=f"q_{item['id']}")
                u_edit = c3.text_input("Uni:", item['orc_item_unidade'], key=f"u_{item['id']}")
